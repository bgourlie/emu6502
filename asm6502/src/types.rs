...
impl<'a> nom::Slice<std::ops::RangeFrom<usize>> for LexInput<'a> {
    fn slice(&self, range: std::ops::RangeFrom<usize>) -> Self {
        LexInput(self.0.slice(range))
    }
}

impl<'a> InputTake for LexInput<'a> {
    fn take(&self, count: usize) -> Self {
        LexInput(self.0.take(count))
    }

    fn take_split(&self, count: usize) -> (Self, Self) {
        let (left, right) = self.0.take_split(count);
        (LexInput(left), LexInput(right))
    }
}

impl<'a> nom::Compare<&'a str> for LexInput<'a> {
...
impl<'a> nom::Compare<LexInput<'a>> for LexInput<'a> {
    fn compare(&self, t: LexInput<'a>) -> nom::CompareResult {
        self.0.compare(t.0)
    }

    fn compare_no_case(&self, t: LexInput<'a>) -> nom::CompareResult {
        self.0.compare_no_case(t.0)
    }
}

impl<'a> nom::InputLength for LexInput<'a> {
    fn input_len(&self) -> usize {
        self.0.input_len()
    }
}

impl<'a> nom::UnspecializedInput for LexInput<'a> {}

impl<'a> InputIter for LexInput<'a> {
    type Item = char;
...
#[derive(Copy, Clone, Debug, PartialEq)]
pub struct LexInput<'a>(pub &'a str);

impl<'a> From<&'a str> for LexInput<'a> {
    fn from(string: &'a str) -> Self {
        LexInput(string)
    }
}

impl<'a> nom::Compare<LexInput<'a>> for &'a str {
    fn compare(&self, t: LexInput<'a>) -> nom::CompareResult {
        self.compare(t.0)
    }
